    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       String id=request.getParameter("id_empleado");
		...
		try{
			...
			if(registrar.RegistrarMaestro(id_empleado, nombre,correo)){
                            validacion="Registro Exitoso";
                            request.getSession().setAttribute("validar", validacion);
                            response.sendRedirect("Coordinadora/AgregarMaestro.jsp");
                               
			}
                       
                      
                        if(registrar.RegistrarUsuario(id_empleado, 1, usuario, contraseña));
